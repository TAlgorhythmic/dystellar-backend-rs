...
* }
*/
async fn loginsession(req: Request<Incoming>) -> Result<Response<Full<Bytes>>, BackendError> {
...
* }
*/
async fn login_existing(req: Request<Incoming>) -> Result<Response<Full<Bytes>>, BackendError> {
    let body = get_body_json(HttpTransaction::Req(req)).await?;
...
* }
*/
async fn login(req: Request<Incoming>) -> Result<Response<Full<Bytes>>, BackendError> {
    let pend = &PENDING;
